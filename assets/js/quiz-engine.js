...
(function (window) {
    ...
    class QuizEngine {
        ...
        refreshElements() {
            this.page = {
                ...
                currentElement: document.getElementById('current-question-num') || document.getElementById('current-q') || document.getElementById('q-current'),
                totalElement: document.getElementById('total-questions-num') || document.getElementById('total-q') || document.getElementById('q-total'),
                scoreElement: document.getElementById('score-display') || document.getElementById('score') || document.getElementById('quiz-score'),
                ...
                feedbackElement: document.getElementById('feedback-box') || document.getElementById('feedback') || document.getElementById('quiz-feedback') || document.getElementById('quiz-result'),
                nextButton: document.getElementById('next-btn') || document.getElementById('next-q-btn'),
                retryButton: document.getElementById('retry-btn') || document.getElementById('restart-btn'),
                cardElement: document.querySelector('.quiz-container-card') || document.querySelector('.question-card') || document.querySelector('.quiz-card') || document.getElementById('quiz-body')
            };
        }
        ...
        normalizeQuestions(rawData) {
            ...
            return list.map(q => {
                if (!q) return q;
                const cleanQ = { ...q };
                if (typeof cleanQ.question === 'string') {
                    // Strip leading numbers like "1. ", "10- ", "5) " so shuffled questions don't conflict with counter
                    cleanQ.question = cleanQ.question.replace(/^\d+[\.\-\)]\s*/, '').trim();
                }
                return cleanQ;
            });
        }
        ...
        renderQuestion() {
            ...
            if (this.page.questionElement) {
                ...
                const hasAr = window.BiDi && typeof window.BiDi.hasArabic === 'function' 
                    ? window.BiDi.hasArabic(qText) 
                    : /[\u0600-\u06FF]/.test(qText);
                const formattedQ = formatBidi(qText);
                this.page.questionElement.style.direction = hasAr ? 'rtl' : 'ltr';
                this.page.questionElement.style.textAlign = 'center';
                this.page.questionElement.innerHTML = `
                    ...
                    <button class="audio-btn" style="margin-${hasAr ? 'right' : 'left'}: 10px; display: inline-flex; vertical-align: middle; cursor: pointer;" title="استمع للسؤال" type="button">🔊</button>
                `;
        ...
        renderOptions(question) {
            ...
            optionsList.forEach(option => {
                ...
                const isArabic = window.BiDi && typeof window.BiDi.hasArabic === 'function' 
                    ? window.BiDi.hasArabic(optionText) 
                    : /[\u0600-\u06FF]/.test(optionText);
                ...
                button.style.direction = isArabic ? 'rtl' : 'ltr';
                button.style.textAlign = isArabic ? 'right' : 'left';
        ...
        handleAnswer(option, button, question) {
            ...
            if (isCorrect) {
                ...
                // Dispatch decoupled event for vocabulary mastery or analytics subsystems
                try {
                    let correctAnswerText = '';
                    if (Array.isArray(question.options)) {
                        const corr = question.options.find(opt => (typeof opt === 'object' && opt !== null) ? opt.isCorrect : (String(opt).trim().toLowerCase() === String(question.answer || '').trim().toLowerCase()));
                        if (corr) {
                            correctAnswerText = (typeof corr === 'object') ? (corr.text || '') : String(corr);
                        }
                    }
                    if (!correctAnswerText && question.answer) {
                        correctAnswerText = String(question.answer);
                    }
                    ...
                    // Fail silently to ensure quiz engine flow is never interrupted
                }
            }
    ...
    // Universal Static Bridge on QuizEngine class
    QuizEngine.init = function(data) {
        if (globalQuizEngine && data) {
            globalQuizEngine.loadQuestions(data);
        }
    };
    QuizEngine.loadQuestions = function(data) {
        if (globalQuizEngine && data) {
            globalQuizEngine.loadQuestions(data);
        }
    };
    QuizEngine.nextQuestion = function() {
        globalQuizEngine.nextQuestion();
    };
    QuizEngine.restartQuiz = function() {
        globalQuizEngine.restartQuiz();
    };
    QuizEngine.retryQuiz = function() {
        globalQuizEngine.restartQuiz();
    };
    QuizEngine.prototype.init = function(data) {
